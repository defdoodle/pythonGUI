...
def add_file() :
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:/Users/doodle/python_project/")
    for file in files :
...
def merge_image() :
    ...
    try :
        ...
        messagebox.showinfo("alarm", "Work's DONE!!!")
    except Exception as err :
        messagebox.showerror("ERRPR", err)
...
def start() :
    ...
    # 파일 목록 확인
    if list_file.size() == 0 :
        messagebox.showwarning("warning", "add image!!!")
        return

    # 저장 경로 확인
    if len(txt_dest_path.get()) == 0 :
        messagebox.showwarning("warning", "add path!!!")
        return
...
btn_start = Button(frame_run, padx=4, pady=5, text="시작", width=12, command=start)
